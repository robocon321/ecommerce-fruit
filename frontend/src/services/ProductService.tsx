...
import {
  ...
  ProductDiscountSummaryResponse,
  ProductPageResponse,
  ProductSummaryResponse,
} from "@/types/response/ProductResponse";
...
export const getProductsByCategories = async (
  ...
  try {
    const response = await fetch(
      `${process.env.BACKEND_URL}/product/getByCategories?` +
        new URLSearchParams(query),
      {
        ...
        next: {
          revalidate: 3600,
          tags: ["product", "get-product", "get-product-by-categories"],
        },
      }
    );
...
export const getProductsByRatingCount = async (
  pageRequest?: PageRequest
): Promise<ProductSummaryResponse[]> => {
  const query: any = {};
  if (pageRequest) {
  ...
  try {
    const response = await fetch(
      `${process.env.BACKEND_URL}/product/getByRatingCount?` +
        new URLSearchParams(query),
      {
        method: "GET",
        cache: "force-cache",
        next: {
          revalidate: 3600,
          tags: ["product", "get-product", "get-product-by-rating-count"],
        },
      }
    );
...
export const getProductsByRatingAverage = async (
  pageRequest?: PageRequest
): Promise<ProductSummaryResponse[]> => {
  const query: any = {};
  if (pageRequest) {
  ...
  try {
    const response = await fetch(
      `${process.env.BACKEND_URL}/product/getByRatingAvg?` +
        new URLSearchParams(query),
      {
        method: "GET",
        cache: "force-cache",
        next: {
          revalidate: 3600,
          tags: ["product", "get-product", "get-product-by-rating-avg"],
        },
      }
    );
...
export const getProductsByDiscount = async (
  pageRequest?: PageRequest
): Promise<ProductDiscountSummaryResponse[]> => {
  const query: any = {};
  if (pageRequest) {
    if (pageRequest.size) {
      query.size = pageRequest.size.toString();
  ...
  try {
    const response = await fetch(
      `${process.env.BACKEND_URL}/product/getByDiscount?` +
        new URLSearchParams(query),
      {
        method: "GET",
        cache: "force-cache",
        next: {
          revalidate: 3600,
          tags: ["product", "get-product", "get-product-by-discount"],
        },
      }
    );
